...
import {
  RequestRefund,
  RefundRequestStatus,
} from '@/entities/request-refund.entity';
...
export class WithdrawalService {
  ...
  constructor(
    ...
    @InjectRepository(RequestRefund)
    private readonly refundRequestRepository: Repository<RequestRefund>,
  ...
  async requestRefundByAdmin(
    orderId: string,
    body: RequestRefundDto,
  ): Promise<RequestRefund> {
    ...
    if (!payment) {
      throw new NotFoundException('Payment not found for the order');
    }
    ...
    return await this.refundRequestRepository.save(request);
  }
}
