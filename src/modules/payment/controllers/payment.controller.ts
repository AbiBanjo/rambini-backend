...
export class PaymentController {
  ...
  async getSavedCards(
    @GetUser() user: User,
  ): Promise<{ stripe: any[]; paystack: any[] }> {
    const stripeCards = await this.stripePaymentService.getUserSavedCards(
      user.id,
    );
    const paystackCards = await this.paystackPaymentService.getUserSavedCards(
      user.id,
    );
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad request - Paystack configuration missing',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({
    status: 500,
    description: 'Internal server error - Failed to fetch banks',
  })
  ...
  @Get('transactions')
  @ApiOperation({
    summary: 'Get user transaction history with pagination and filters',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad request - Invalid payment data or insufficient balance',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad request - Invalid funding data',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  async fundWallet(
    @GetUser() user: User,
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  async getWalletBalance(@GetUser() user: User): Promise<WalletBalanceDto> {
  ...
  async getAllWalletDetails(): Promise<WalletBalanceDto[]> {
    return await this.paymentService.getAllWalletDetails();
  }
  ...
  @ApiOperation({
    summary: 'Verify wallet funding payment with payment provider',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    summary:
      'Complete wallet funding after successful external payment (for webhooks)',
  })
  @ApiParam({ name: 'reference', description: 'Wallet funding reference' })
  @ApiResponse({
    status: 200,
  ...
  async getTransactionById(
    ...
    return await this.walletPaymentService.getTransactionById(
      user.id,
      transactionId,
    );
  }
  ...
  @ApiOperation({
    summary: 'Create Stripe Setup Intent for saving a card (no charge)',
  })
  ...
  @ApiOperation({
    summary: 'Attach a payment method to a Stripe customer and set default',
  })
  async attachStripePaymentMethod(
    ...
    return await this.stripePaymentService.attachPaymentMethodLight(
      body.customer_id,
  ...
  @ApiOperation({
    summary:
      'Initialize Paystack transaction that will save card (authorization_code)',
  })
  ...
  @ApiOperation({
    summary:
      'Verify Paystack reference and return authorization_code to store client-side',
  })
  async paystackSaveAuthorization(
    ...
    return await this.paystackPaymentService.extractAuthorizationFromReference(
      body.reference,
    );
  }
  ...
  @ApiOperation({
    summary:
      'Charge a saved Paystack card (provide authorization_code + email)',
  })
