...
import { RequestRefundDto } from '../dto/admin-refund.dto';
...
export class WithdrawalController {
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        message: {
          type: 'string',
          example: 'Withdrawal OTP sent to your email successfully',
        },
      },
    },
  })
  @ApiResponse({
    status: 400,
    description:
      'Bad request - insufficient balance or active withdrawal exists',
  })
  ...
  async generateWithdrawalOTP(
    ...
    return await this.withdrawalService.generateWithdrawalOTP(
      user.id,
      withdrawalOtpRequest.amount,
    );
  }
  ...
  async requestWithdrawal(
    ...
    return await this.withdrawalService.requestWithdrawal(
      req.user.id,
      withdrawalRequest,
    );
  }
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad request - duplicate bank account or invalid data',
  })
  async createBank(
    @GetUser() user: User,
...
export class AdminWithdrawalController {
  ...
  @Post(':orderId')
  @ApiOperation({ summary: 'Request a refund for a user order' })
  async requestRefund(
    @Param('orderId') orderId: string,
    @Body() body: RequestRefundDto,
  ) {
    return await this.withdrawalService.requestRefundByAdmin(orderId, body);
  }
}
