  ...
  export class AdminVendorController {
    ...
    @ApiOperation({ 
      ...
      description: 'Retrieves all vendors with complete address information and formatted address details.'
    })
    @ApiResponse({
      status: 200,
    ...
    @ApiOperation({ 
      ...
      description: 'Retrieves a specific vendor with complete address information and formatted address details.'
    })
    ...
    @ApiOperation({ 
      ...
      description: 'Activates a vendor profile. Returns the activated vendor with formatted address details.'
    })
    @ApiParam({ name: 'vendorId', description: 'Vendor ID' })
    @ApiResponse({
      status: 200,
    ...
    @ApiOperation({ 
      ...
      description: 'Deactivates a vendor profile. Returns the deactivated vendor with formatted address details.'
    })
    @ApiParam({ name: 'vendorId', description: 'Vendor ID' })
    @ApiResponse({
      status: 200,
  ...
  @ApiOperation({ 
    ...
    description: 'Removes duplicate/corrupted data from address_line_2 fields. Run once to fix database.'
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  async cleanVendorAddresses(): Promise<{ message: string; cleaned: number; total: number }> {
  ...
  @ApiOperation({ 
    ...
    description: 'Removes duplicate/corrupted data from a specific vendor\'s address_line_2.'
  })
  @ApiParam({ name: 'vendorId', description: 'Vendor ID' })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Vendor not found' })
  async cleanSingleVendorAddress(@Param('vendorId') vendorId: string): Promise<{ message: string }> {
    ...
    @ApiOperation({ 
      ...
      description: 'Permanently deletes a vendor profile and the associated user account.'
    })
    @ApiParam({ name: 'vendorId', description: 'Vendor ID' })
    @ApiResponse({ 
      status: 200, 
